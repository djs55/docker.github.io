...
- Removed any dangling `/usr/local/bin/docker-machine` symlinks which avoids custom installs of `docker-machine` being accidentally deleted in future upgrades. Note that if you have installed Docker Machine manually, then the install might have followed the symlink and installed Docker Machine in `/Applications/Docker.app`. In this case, you must manually reinstall Docker Machine after installing this version of Docker Desktop. See [docker/for-mac#4208](https://github.com/docker/for-mac/issues/4208).
...
- Kubernetes: persistent volumes created by claims are now stored in the virtual machine. See [docker/for-win#5665](https://github.com/docker/for-win/issues/5665).
